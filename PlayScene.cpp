...
{
	enemies->Update(*player, *collision, static_cast<float>(ctx->fps_controller->getDeltaTime()), map->getModelHandle());
	...
	charge_speed = player->getCurrentState() != State::SPECIAL_ATTACK ? 15.0f / (60.0f * 15.0f) : 0.0f;
...
void PlayScene::Draw()
{
	...
	if (alpha > 0) {
		...
		ctx->font->DrawFormatChar(
			static_cast<int>(Config::SCREEN_WIDTH / 3.0f),
			static_cast<int>(Config::SCREEN_HEIGHT / 3.0f),
			"Run!!\n", static_cast<int>(FontSize::font1_Size_180), Config::COLOR_RUN_MESSAGE);
